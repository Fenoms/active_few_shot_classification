...
class miniImagenet():
    ...
    def get_tra_batch(self):
        ...
        support_dir = data_dir +'tra_support_data_mini/'
        query_dir = data_dir +'tra_query_data_mini/'

        # class_ = os.listdir(img_dir)
        ...
        query_x = np.zeros((self.batch_size, self.query_size * self.ways, self.data_shape[0],
                            self.data_shape[1], self.data_shape[2]), dtype=np.float32)
        ...
        for i in range(self.batch_size):
            ...
            for j in range(5):
                ...
                support_img_dir = support_dir + str(j)
                query_img_dir = query_dir + str(j)
    ...
    def get_val_test_batch(self, mode):
    ...
    def get_cand_data(self, c, query_img_name):
        ...
        support_set_y = np.zeros((self.batch_size, self.ways, self.shots), dtype=np.uint8)

        query_x = np.zeros((self.batch_size, self.query_size*self.ways, self.data_shape[0], self.data_shape[1], self.data_shape[2]),
                             dtype=np.float32)
        query_y = np.zeros((self.batch_size, self.query_size*self.ways), dtype=np.uint8)
