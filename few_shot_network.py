...
def building_block(inputs, filters, is_training, projection_shortcut, strides, data_format, dropout_prob):
    ...
    inputs = tf.layers.dropout(inputs=inputs, rate=dropout_prob)
...
def bottleneck_block(inputs, filters, is_training, projection_shortcut, strides, data_format, dropout_prob):
...
def block_layer(inputs, filters, block_fn, blocks, strides, is_training, name, data_format, dropout_prob):
    ...
    inputs = block_fn(inputs, filters, is_training, projection_shortcut, strides,
                      data_format, dropout_prob)

    for _ in range(1, blocks):
        inputs = block_fn(inputs, filters, is_training, None, 1, data_format, dropout_prob)

    return tf.identity(inputs, name)


def embeddingResNet_generator(block_fn, layers, dropout_prob, data_format=None):
...
def comparisonNet_generator(block_fn, dropout_prob, data_format=None):
    if data_format == None:
        data_format = 'channels_first' if tf.test.is_built_with_cuda() else 'channels_last'
    ...
    def model(inputs, is_training):
        ...
        #                      blocks=1, strides=2, is_training = is_training, name='compare_3',
        #                      data_format=data_format, dropout_prob=dropout_prob)
        #
        # inputs = block_layer(inputs=inputs, filters=64, block_fn=block_fn,
        #                      blocks=1, strides=2, is_training=is_training, name='compare_4',
        #                      data_format=data_format, dropout_prob=dropout_prob)

        inputs = tf.layers.average_pooling2d(inputs=inputs, pool_size=2, strides=2, data_format=data_format,
                                             name='final_pool')
...
class FewshotsNet:
    ...
    def __init__(self, support_set_images, support_set_labels, query_images, query_labels, data_format, dropout_prob,
                 is_training, batch_size, learning_rate):
        ...
        self.e = embeddingResNet_generator(block_fn=building_block, layers=[2, 2, 2, 2], data_format=data_format,
                                           dropout_prob=dropout_prob)
        self.c = comparisonNet_generator(block_fn=building_block, data_format=data_format, dropout_prob=dropout_prob)
    ...
    def loss(self):
        ...
        with tf.name_scope('losses'):
            [_, num_classes, num_samples_per_class, h, w, c] = self.support_set_images.get_shape().as_list()
            [b, query_size] = self.query_labels.get_shape().as_list()
            support_set_images = tf.reshape(self.support_set_images,
                                            shape=[b * num_classes * num_samples_per_class, h, w, c])
            support_set_labels = tf.reshape(self.support_set_labels, shape=[b, num_classes * num_samples_per_class])
            # self.support_set_labels = tf.one_hot(self.support_set_labels, num_classes)
            ...
            support_images_embeddings = self.e(inputs=support_set_images, is_training=self.is_training)
            [_, _, d, f] = support_images_embeddings.get_shape().as_list()
            support_images_embeddings = tf.reshape(support_images_embeddings,
                                                   shape=[b, num_classes * num_samples_per_class, d, d, f])
            ...
            support_images_embeddings = tf.expand_dims(support_images_embeddings, axis=1)
            # shape of support_images_embeddings: [batch_size, 1, num_classes* num_samples_per_class, d, d, c]
            support_images_embeddings = tf.tile(support_images_embeddings, [1, query_size, 1, 1, 1, 1])
            # shape of support_images_embeddings: [batch_size, num_classes* num_samples_per_class, query_size, d, d, c]
            query_images = tf.reshape(self.query_images, shape=[b * query_size, h, w, c])

            # for image in tf.unstack(self.query_images, axis = 0):
            #     image_embedding = self.e(inputs = image, is_training = self.is_training)
            #     query_images_embeddings.append(image_embedding)
            query_images_embeddings = self.e(inputs=query_images, is_training=self.is_training)
            query_images_embeddings = tf.reshape(query_images_embeddings, shape=[b, query_size, d, d, f])
            ...
            query_images_embeddings = tf.expand_dims(query_images_embeddings, axis=2)
            # shape of query_images_embeddings: [batch_size, query_size, 1, d, d, c]
            query_images_embeddings = tf.tile(query_images_embeddings,
                                              [1, 1, num_classes * num_samples_per_class, 1, 1, 1])
            # shape of query_images_embeddings: [batch_size,  query_size, num_classes* num_samples_per_class, d, d, c]

            inputs = tf.concat(values=[support_images_embeddings, query_images_embeddings], axis=-1)
            # shape of inputs: [batch_size, query_size, num_classes* num_samples_per_class, d, d, 2*c]

            inputs = tf.reshape(inputs,
                                shape=[self.batch_size * num_classes * num_samples_per_class * query_size, d, d, -1])
            ...
            scores = tf.reshape(scores, shape=[self.batch_size, query_size, num_classes * num_samples_per_class])

            support_set_labels = tf.expand_dims(support_set_labels, axis=1)

            support_set_labels = tf.tile(support_set_labels, [1, query_size, 1])
            print("support_set_labels shape", support_set_labels.get_shape())

            query_labels_1 = self.query_labels

            query_labels = tf.expand_dims(self.query_labels, axis=2)
            query_labels = tf.tile(query_labels, [1, 1, num_classes * num_samples_per_class])
            ...
            uncertainty = msq_losses * self.batch_size
            ...
            support_set_labels_ = tf.reshape(self.support_set_labels, shape=[b, num_classes * num_samples_per_class])
            index_prediction = tf.cast(tf.argmax(tf.reduce_sum(scores_1, axis=3), axis=2), tf.int64)

            correct_prediction = []
            for i in range(b):
                for j in range(query_size):
                    correct_prediction.append(
                        support_set_labels_[i][tf.multiply(index_prediction[i][j], num_samples_per_class)])

            correct_prediction = tf.cast(tf.reshape(tf.stack(correct_prediction), shape=[b, query_size]), tf.uint8)
            ...
            return {'loss': msq_losses, 'accuracy': accuracy, 'uncertainty': uncertainty}
